...
-- @module tests.modules.middleware.registry_test
...
_M.tests = {
    {
        ...
        func = function()
            ...
            test_utils.assert_not_nil(reg.request_id, "Request ID middleware should be in registry")
            test_utils.assert_equals(true, reg.request_id.multi_phase, "Request ID should be multi-phase")
            test_utils.assert_not_nil(reg.request_id.phases.access, "Request ID should have access phase")
            test_utils.assert_not_nil(reg.request_id.phases.header_filter, "Request ID should have header_filter phase")
            test_utils.assert_not_nil(reg.request_id.phases.log, "Request ID should have log phase")
            
            -- Verify phases
            local phases = registry.get_phases()
        ...
        func = function()
            ...
            local ok = pcall(function()
                for name, config in pairs(invalid_reg) do
                    registry.register_middleware(name, config)
                end
            end)
